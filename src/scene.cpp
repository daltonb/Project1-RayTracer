...
int scene::loadObject(string objectid){
  ...
  if(id!=objects.size()){
    ...
    utilityCore::safeGetline(fp_in,line);
    newObject.hasTexture = false;
    vector<string> tokens = utilityCore::tokenizeString(line);
    if (tokens.size()) {
      if(strcmp(tokens[0].c_str(), "TEXTURE")==0) {
        newObject.hasTexture = true;
        ifstream infile(string("..\\..\\scenes\\") + tokens[1]);
        cout << "Loading texture file: " << (string("..\\..\\scenes\\") + tokens[1]) << endl;
        string filetype;
        int numRows;
        int numCols;
        int res;
        infile >> filetype >> numRows >> numCols >> res;
        cout << "Rows: " << numRows << endl;
        cout << "Cols: " << numCols << endl;
        newObject.texture = new glm::vec3[numRows*numCols];
        newObject.textureRes = glm::vec2(numRows, numCols);
        for (int i=0; i<numRows; i++) {
          for (int j=0; j<numCols; j++) {
            int index = i*numCols + j;
            float x, y, z;
            infile >> x >> y >> z;
            newObject.texture[index] = glm::vec3(x/255.0f, y/255.0f, z/255.0f);
          }
        }
        utilityCore::safeGetline(fp_in,line);
      }
